...
- [x] Bazzite kernel, in other words, ogc kernel must be installed in bazzfin project using the bluebuild system. How can one achieve that ? Search the web to determine if that is possible. Look at the Containerfile from the bazzite project in github that I attached here. I know the akmods module can use the ogc but likely only means that nvidia akmods are built against bazzite kernel. Implement a likely comprehensive solution that covers all bases.
- [x] Review and audit the above response to make sure there are no errors and issues. And rewrite your latest response again in its entirety. Also if I am using the same base ghcr.io/ublue-os/base-main, how would the initramfs module in recipe.yml work correctly with the new ogc kernel given that typically only the kernel from the base image is active in the github workflow. This is similar to how the command `sudo dracut -f` in a normal fedora installation would only build against the running kernel even if you installed updated kernels since these updated kernel is only available after reboot. Also add the following features from bazzite: Bazzite also ships extra kargs (kernel command-line tweaks), zram tuning, and HDR-related userspace bits that ride alongside its kernel. kargs should be added using the kargs modules from blue-build/modules
...
- [ ] integrate bazzite gaming related features and functionalities
- [ ] integrate bazzite packages for system management and bazzite tooling system.
...
- [ ] make sure important selinux packages since ublue-os base-image might not install them by default
...
- [ ] Either use https://github.com/ublue-os/uupd or ujust update bazzite ujust documentation
- [ ]
