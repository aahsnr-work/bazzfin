...
- [x] make sure important selinux packages since ublue-os base-image might not install them by default (done 2026-09-15: audited the base image -- the core stack is already present, i.e. selinux-policy-targeted, policycoreutils + policycoreutils-python-utils, checkpolicy, libselinux-utils, container-selinux, flatpak-selinux, python3-setools, rpm-plugin-selinux -- and added what Bazzite ships beyond it via the bazzite system-management dnf module: setroubleshoot/-server/-plugins, setools-console, udica, plus ublue-os-selinux-workarounds (composefs execmem policy mitigation). The build also applies Bazzite's /var/lib/selinux -> /etc/selinux patch to setroubleshoot's util.py, required on ostree/bootc systems)
